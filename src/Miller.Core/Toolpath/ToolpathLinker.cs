...
// Joins passes into one toolpath. A pass is a chain of feed segments a strategy produced; a group
// holds the passes of one level or operation, and groups keep their order (a level is finished
// before the next), while the passes inside a group are ordered by PassOrdering. Between passes
// whose end and start are farther apart than one cell, or whose straight join would gouge the
// effective tip map, the tool retracts to safe Z, rapids over and plunges; otherwise the passes are
// joined by one feed. The program starts above the first pass at safe Z and ends with a retract.
public static class ToolpathLinker
{
    public static Toolpath Link(IReadOnlyList<Toolpath> passes, CuttingParameters parameters, float safeZ, HeightMap effectiveTip)
        => Link(new[] { passes }, parameters, safeZ, effectiveTip);

    public static Toolpath Link(IReadOnlyList<IReadOnlyList<Toolpath>> groups, CuttingParameters parameters, float safeZ, HeightMap effectiveTip)
    {
        ...
        {
            foreach (var pass in PassOrdering.Order(group, position, parameters.Direction == MillingDirection.Zigzag))
            {
